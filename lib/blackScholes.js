...
/**
 * Underlying price at which the position breaks even after holding `daysElapsed` days.
 * This is the price where option value == entry premium, i.e. P&L crosses zero.
 * Returns null if no crossing exists within the search bracket.
 */
export function breakEvenStockAt({
  S, K, daysToExpiry, daysElapsed, sigma, r = 0, q = 0, optionType, entryPremium
}) {
  if (!(sigma > 0) || !(entryPremium > 0) || !(daysToExpiry > daysElapsed)) return null
  const val = (px) => optionValueAt({
    S: px, daysElapsed, sigma, K, daysToExpiry, r, q, optionType
  })
  let lo = S * 0.5
  let hi = S * 2.0
  const fLo = val(lo) - entryPremium
  const fHi = val(hi) - entryPremium
  if (fLo === 0) return lo
  if (fHi === 0) return hi
  if ((fLo > 0) === (fHi > 0)) return null
  for (let i = 0; i < 100; i++) {
    const mid = (lo + hi) / 2
    const fMid = val(mid) - entryPremium
    if ((fMid > 0) === (fHi > 0)) hi = mid
    else lo = mid
  }
  return (lo + hi) / 2
}
